	...
	def Grand_Analysis(self):
		...
		if self.meta_analysis[0]:
			meta_stats = ["constants", "axion_power", "modulation", "BS", "consolidation", "cavity_lorentz", "bin_stats", 
							...
							'picking_og_signal', 'dividing_structure', 'consolidation_time', 'scans_in_time', 'class_init_time', 'optimal_weight_sum_consolidation_time', 
							...
							'axion_fit_significance_consolidation_time', 'scans_in_grand_spectra_addition_time', 'power_sensitivity_consolidation_time',
							'init_scans_in_class', 'init_grand_spectra', 'init_grand_spectra_in_class', 'growing_grand_spectra', 'reinit_grand_spectra_in_class']
		...
		for key in self.keys:
			...
			if not scan.attrs['cut']:
				add_subtract_scan('add', self.analysis_results[key], self.grand_spectra_group, key, **params)
			elif scan.attrs['cut'] and scan.attrs['cut_reason']!="scan outside 645 to 685 MHz range":
				self.ncut += 1
				add_subtract_scan('subtract', self.analysis_results[key], self.grand_spectra_group, key, **params)
		...
		if self.meta_analysis[0]:
			...
			e = "" #"average time to reinitialize grand spectra in coaddition class is {0:03f}".format(avg_reinit_GS_in_cl)
			...
			f = "total time to initialize scans in coaddition class is {0:03f}".format(total_init_scans_in_class_time)
			g = "total time to initialize grand_spectra is {0:03f}".format(total_init_GS)
			h = "total time to initialize grand spectra in coaddition class is {0:03f}".format(total_init_GS_in_cl)
			i = "" #"total time to grow grand spectra is {0:03f}".format(total_growing_GS)
			j = "" #"total time to reinitialize grand spectra in coaddition class is {0:03f}".format(total_reinit_GS_in_cl)
			...
			,s49,s50,s51,s52,s53,s54,s55,s56,s57, f,g,h,i,j]
			for i in strings:
